#!/bin/bash
# CMT ZIVPN PRO - NEON GRID & DYNAMIC EXPIRY EDITION
set -euo pipefail
...
<style>
    ...
    /* ✅ Neon Style Grid boxes */
    .grid-info { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 15px; }
    .info-box { background: var(--card); padding: 12px 5px; text-align: center; border: 2px solid var(--cyan); border-radius: 12px; box-shadow: 0 0 15px var(--cyan); transition: 0.3s; }
    
    /* Action Grid Section */
    .action-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px; }
    .action-box { background: var(--card); padding: 15px 5px; border-radius: 12px; border: 2px solid rgba(0, 212, 255, 0.3); text-align: center; cursor: pointer; transition: 0.3s; }
    .action-box:hover { border-color: var(--cyan); box-shadow: 0 0 15px var(--cyan); transform: translateY(-3px); }
    .action-box i { font-size: 1.5em; margin-bottom: 8px; display: block; }
    .action-box span { font-size: 0.75em; font-weight: bold; }
    ...
    .main-btn { background: linear-gradient(90deg, #ff0000, #ffaa00, #00d4ff); padding: 12px; border: none; border-radius: 12px; color: #fff; width: 100%; font-weight: bold; cursor: pointer; text-shadow: 1px 1px 5px #000; }
    
    .table-card { background: var(--card); border-radius: 12px; border: 2px solid var(--cyan); overflow-x: auto; padding: 10px; box-shadow: 0 0 15px rgba(0,212,255,0.2); }
...
{% else %}
    <div class="header">
        <img src="{{logo}}" class="logo-img" onerror="this.src='https://raw.githubusercontent.com/hninpo01/CMT/main/logo.png'">
    ...
    <div class="container">
        <h2 class="rainbow-text" style="text-align:center; margin-bottom:15px; font-size:1.5em;">CMT ZIVPN PRO PANEL</h2>
        <div class="grid-info">
            <div class="info-box" style="border-color:var(--cyan);"><small>CPU</small><div style="color:var(--cyan); font-weight:bold;">{{sys.cpu}}%</div></div>
            <div class="info-box" style="border-color:var(--yellow); box-shadow: 0 0 15px var(--yellow);"><small>RAM</small><div style="color:var(--yellow); font-weight:bold;">{{sys.ram}}%</div></div>
            <div class="info-box" style="border-color:var(--green); box-shadow: 0 0 15px var(--green);"><small>DISK</small><div style="color:var(--green); font-weight:bold;">{{sys.disk}}%</div></div>
        </div>

        <div class="grid-info">
            <div class="info-box" style="border-color:var(--purple); box-shadow: 0 0 15px var(--purple);"><small>အသုံးပြုသူ</small><div style="color:var(--purple); font-weight:bold;">{{users|length}}</div></div>
            <div class="info-box" style="border-color:var(--green);"><small>အွန်လိုင်း</small><div style="color:var(--green); font-weight:bold;">{{active_count}}</div></div>
            <div class="info-box" style="border-color:var(--yellow);"><small>ဝန်အား</small><div style="color:var(--yellow); font-weight:bold;">12%</div></div>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span style="color:{{ 'var(--green)' if u.online else '#ff4444' }}"><i class="fas fa-circle status-dot"></i>{{ 'Online' if u.online else 'Offline' }}</span></td>
                        <td>
    ...
    <div id="addModal" class="modal"><div class="modal-content">
        ...
        <button onclick="toggleModal('addModal')" style="background:none;border:none;color:#aaa;margin-top:15px;cursor:pointer;">ပိတ်မည်</button>
    </div></div>
    ...
    <div id="supportModal" class="modal"><div class="modal-content">
        ...
        <button onclick="toggleModal('supportModal')" style="background:none;border:none;color:#aaa;margin-top:10px;cursor:pointer;">ပိတ်မည်</button>
    </div></div>

    <div id="renewModal" class="modal"><div class="modal-content">
        <h3 class="rainbow-text">သက်တမ်းပြင်ဆင်ရန်</h3>
        <p style="font-size:0.7em; color:#aaa;">(ရက်ပေါင်းထည့်ပါ - လျော့ချချင်ရင် ဂဏန်းရှေ့မှာ - ထည့်ပါ)</p>
        <form method="post" action="/renew">
            <input type="hidden" name="user" id="renewUser">
            <input name="days" placeholder="ရက်ပေါင်း (ဥပမာ ၃၀ သို့မဟုတ် -၅)" required>
            <button class="main-btn">အတည်ပြုသည်</button>
        </form>
        <button onclick="toggleModal('renewModal')" style="background:none;border:none;color:#aaa;margin-top:15px;cursor:pointer;">ပိတ်မည်</button>
    </div></div>
...
<script>
    ...
    function startClock(){ 
        setInterval(function(){ 
            ...
            var th=new Date(utc+25200000); 
            var h=th.getHours(),m=th.getMinutes(),s=th.getSeconds(),ap=h>=12?'PM':'AM'; 
            h=h%12||12; h=h<10?'0'+h:h; m=m<10?'0'+m:m; s=s<10?'0'+s:s; 
            document.getElementById('liveTime').innerHTML=h+':'+m+':'+s+' '+ap; 
            document.getElementById('liveDate').innerHTML=th.toDateString(); 
        }, 1000); 
    }

    /* ✅ Particle Animation (ကြိုးချောင်းလေးများ) */
...
<style>
    ...
    .card { background: rgba(16, 22, 42, 0.9); padding: 15px; border-radius: 12px; border: 2px solid #00d4ff; box-shadow: 0 0 10px #00d4ff; margin-bottom: 20px; }
    ...
    <div class="card">
        ...
        <form method="post" action="/update_pass">
            <input name="old_u" placeholder="Admin အမည်ဟောင်း">
...
echo -e "\n✅ Professional Update Done! Panel: http://$IP:8080"
