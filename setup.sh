#!/bin/bash
# CMT ZIVPN PRO - PROFESSIONAL GRID ACTION UI (STABLE)
set -euo pipefail
...
<style>
    ...
    /* ✅ Quick Action Grid (As per User request) */
    .action-section { margin-bottom: 25px; }
    .section-title { font-size: 0.9em; margin-bottom: 10px; color: #aaa; display: flex; align-items: center; gap: 8px; }
    .action-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
    .action-box { background: var(--card); padding: 15px 5px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); text-align: center; cursor: pointer; transition: 0.3s; }
    ...
    .modal-content { background: var(--card); margin: 20% auto; padding: 25px; width: 85%; max-width: 350px; border-radius: 20px; border: 2px solid var(--cyan); text-align: center; }
    input { width: 100%; padding: 12px; margin: 10px 0; background: #000; color: #fff; border: 1.5px solid var(--cyan); border-radius: 10px; outline: none; }
...
{% else %}
    ...
    <div class="container">
        <div class="grid-info">
            <div class="info-box"><small>CPU</small><div style="color:var(--cyan);font-weight:bold;">{{sys.cpu}}%</div></div>
            <div class="info-box" style="border-color:var(--yellow); box-shadow: 0 0 10px var(--yellow);"><small>RAM</small><div style="color:var(--yellow);font-weight:bold;">{{sys.ram}}%</div></div>
            <div class="info-box" style="border-color:var(--green); box-shadow: 0 0 10px var(--green);"><small>DISK</small><div style="color:var(--green);font-weight:bold;">{{sys.disk}}%</div></div>
        </div>

        <div class="action-section">
            <div class="section-title"><i class="fas fa-bolt" style="color:var(--yellow);"></i> အမြန်လုပ်ဆောင်ချက်များ</div>
            <div class="action-grid">
                <div class="action-box" onclick="toggleModal('addModal')">
                    <i class="fas fa-user-plus" style="color:var(--green);"></i>
                    <span>အကောင့်သစ်</span>
                </div>
                <div class="action-box" onclick="toggleModal('supportModal')">
                    <i class="fas fa-headset" style="color:var(--cyan);"></i>
                    <span>ဆက်သွယ်ရန်</span>
                </div>
                <div class="action-box" onclick="location.href='/settings'">
                    <i class="fas fa-cog" style="color:var(--purple);"></i>
                    <span>စက်တင်များ</span>
                </div>
            </div>
        </div>

        <div class="table-card">
            <div class="section-title" style="padding-left:10px;"><i class="fas fa-users"></i> အသုံးပြုသူများ စာရင်း</div>
            <table>
                <thead><tr><th>အမည်</th><th>စကားဝှက်</th><th>သက်တမ်း</th><th>Status</th><th>Action</th></tr></thead>
                <tbody>
                    {% for u in users %}
                    <tr>
    ...
    <div id="renewModal" class="modal"><div class="modal-content">
        <h3 class="rainbow-text">သက်တမ်းတိုးရန်</h3>
        <form method="post" action="/renew"><input type="hidden" name="user" id="renewUser"><input name="days" placeholder="ထပ်တိုးမည့်ရက်" required><button class="main-btn">အတည်ပြုသည်</button></form>
...
<style>
    ...
    .card { background: rgba(16, 22, 42, 0.9); padding: 15px; border-radius: 12px; border: 1px solid #00d4ff; margin-bottom: 20px; }
