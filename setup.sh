#!/bin/bash
# CMT ZIVPN PRO - NEON FULL FIX (SEARCH + BOT + SUPPORT)
set -euo pipefail
...
<style>
    :root { --bg: #050810; --card: rgba(16, 22, 42, 0.95); --cyan: #00d4ff; --green: #2ecc71; --purple: #9b59b6; --yellow: #ffaa00; --glow: #ff4500; }
    ...
    .header { background: rgba(0,0,0,0.7); padding: 10px 15px; display: flex; align-items: center; justify-content: space-between; border-bottom: 2px solid var(--cyan); backdrop-filter: blur(10px); }
    ...
    .action-box i { font-size: 1.5em; margin-bottom: 5px; display: block; }
    .modal { display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); }
    .modal-content { background: var(--card); margin: 20% auto; padding: 25px; width: 85%; max-width: 350px; border-radius: 20px; border: 2px solid var(--cyan); box-shadow: 0 0 20px var(--cyan); text-align: center; }
    ...
    .table-card { background: var(--card); border-radius: 12px; border: 2px solid var(--cyan); overflow-x: auto; padding: 10px; }
...
{% else %}
    ...
    <div class="container">
        <h2 class="rainbow-text" style="text-align:center; margin-bottom:15px; font-size:1.4em;">CMT ZIVPN PRO PANEL</h2>
        <div class="grid-info">
            <div class="info-box" style="border-color:var(--cyan);"><small>CPU</small><div style="color:var(--cyan)">{{sys.cpu}}%</div></div>
            <div class="info-box" style="border-color:var(--yellow);"><small>RAM</small><div style="color:var(--yellow)">{{sys.ram}}%</div></div>
            <div class="info-box" style="border-color:var(--green);"><small>DISK</small><div style="color:var(--green)">{{sys.disk}}%</div></div>
        </div>
        <div class="grid-info">
            <div class="info-box" style="border-color:var(--purple);"><small>အသုံးပြုသူ</small><div style="color:var(--purple)">{{users|length}}</div></div>
        ...
        <div style="margin-bottom:15px;"><input type="text" id="userInput" onkeyup="searchUser()" placeholder="ဝယ်သူအမည်ဖြင့် ရှာဖွေရန်..." style="width:100%;padding:12px;background:rgba(0,0,0,0.5);color:#fff;border:1.5px solid var(--cyan);border-radius:10px;"></div>
            ...
            <table id="userTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="color:#ff69b4;">{{u.expires}}<br><small style="color:{{ 'var(--green)' if u.days_left > 5 else '#ff4444' }}">{{u.days_left}} ရက်ကျန်</small></td>
    ...
    <div id="supportModal" class="modal"><div class="modal-content">
        ...
        <a href="{{tg_link}}" target="_blank" style="display:block;padding:12px;margin:10px 0;background:#0088cc;color:#fff;border-radius:10px;text-decoration:none;">Telegram</a>
        <a href="{{fb_link}}" target="_blank" style="display:block;padding:12px;margin:10px 0;background:#1877f2;color:#fff;border-radius:10px;text-decoration:none;">Facebook</a>
        <a href="{{msg_link}}" target="_blank" style="display:block;padding:12px;margin:10px 0;background:linear-gradient(45deg,#00c6ff,#bc00ff);color:#fff;border-radius:10px;text-decoration:none;">Messenger</a>
    ...
    <div id="renewModal" class="modal"><div class="modal-content">
        <h3 class="rainbow-text">သက်တမ်းပြင်ဆင်ရန်</h3>
        <form method="post" action="/renew"><input type="hidden" name="user" id="renewUser"><input name="days" placeholder="ရက်ပေါင်း (ဥပမာ ၃၀ သို့မဟုတ် -၅)" required><button class="main-btn">အတည်ပြုသည်</button></form>
...
<script>
    ...
    function searchUser() { var input, filter, table, tr, td, i; input = document.getElementById("userInput"); filter = input.value.toUpperCase(); table = document.getElementById("userTable"); tr = table.getElementsByTagName("tr"); for (i = 1; i < tr.length; i++) { td = tr[i].getElementsByTagName("td")[0]; if (td) { if (td.textContent.toUpperCase().indexOf(filter) > -1) { tr[i].style.display = ""; } else { tr[i].style.display = "none"; } } } }
    function startClock(){ setInterval(function(){ var n=new Date(); var th=new Date(n.getTime()+(n.getTimezoneOffset()*60000)+25200000); var h=th.getHours(),m=th.getMinutes(),s=th.getSeconds(),ap=h>=12?'PM':'AM'; h=h%12||12; h=h<10?'0'+h:h; m=m<10?'0'+m:m; s=s<10?'0'+s:s; document.getElementById('liveTime').innerHTML=h+':'+m+':'+s+' '+ap; document.getElementById('liveDate').innerHTML=th.toDateString(); }, 1000); }
    const cvs=document.getElementById('bgCanvas'),ctx=cvs.getContext('2d'); let pts=[],hue=0; function init(){cvs.width=window.innerWidth;cvs.height=window.innerHeight;} window.onresize=init; init();
...
def index():
    ...
    return render_template_string(HTML, users=u_list, active_count=sum(1 for u in u_list if u["online"]), logo=OFFICIAL_LOGO, sys=get_sys_info(), tg_link=get_env("SUPPORT_TG"), fb_link=get_env("SUPPORT_FB"), msg_link=get_env("SUPPORT_MSG"))
...
def settings():
    ...
    return render_template_string("<!doctype html><html><head><style>body{background:#050810;color:#fff;font-family:sans-serif;padding:20px;}.card{background:rgba(16,22,42,0.9);padding:15px;border-radius:12px;border:1px solid #00d4ff;margin-bottom:20px;}input{width:100%;padding:12px;margin:8px 0;background:#000;color:#fff;border:1px solid #ff4500;border-radius:10px;box-sizing:border-box;}.btn{background:#00d4ff;padding:12px;border:none;border-radius:10px;width:100%;font-weight:bold;cursor:pointer;color:#000;}</style></head><body><h2>စက်တင်များ</h2><div class='card'><h4>Admin Security</h4><form method='post' action='/update_pass'><input name='old_u' placeholder='Admin အမည်ဟောင်း'><input name='old_p' type='password' placeholder='စကားဝှက်အဟောင်း'><input name='new_p' type='password' placeholder='စကားဝှက်အသစ်'><button class='btn'>Update Admin</button></form></div><div class='card'><h4>Telegram Bot Connect</h4><form method='post' action='/update_tg'><input name='token' placeholder='Bot Token' value='{{token}}'><input name='chat_id' placeholder='Chat ID' value='{{chat_id}}'><button class='btn'>Save Token</button></form></div><a href='/' style='color:#aaa;text-decoration:none;'>Back Home</a></body></html>", token=get_env("TG_TOKEN"), chat_id=get_env("TG_CHAT_ID"))
...
def update_pass():
    if session.get("auth"):
        if hmac.compare_digest(request.form.get("old_u"), get_env("WEB_ADMIN_USER")) and hmac.compare_digest(request.form.get("old_p"), get_env("WEB_ADMIN_PASSWORD")):
...
def add():
    ...
    if os.path.exists("/etc/zivpn/users.json"):
        with open("/etc/zivpn/users.json","r") as f: u_list = json.load(f)
    u_list.insert(0, {"user":u, "password":p, "expires":exp, "port":str(6000+len(u_list))})
