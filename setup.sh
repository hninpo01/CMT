#!/bin/bash
# CMT ZIVPN PRO - FINAL ULTIMATE (CLOCK + ALL FEATURES)
set -euo pipefail
...
<head>
    ...
    <style>
        ...
        .clock-date { font-size: 0.8em; color: rgba(255,255,255,0.7); }
...
{% else %}
    ...
    <div class="header">
        <div style="display:flex;align-items:center;gap:12px;">
            <img src="{{ logo }}">
            <div class="clock-box">
                <div id="liveTime" class="clock-time rainbow-text"></div>
    ...
    <div class="container">
        ...
        <div class="grid-menu">
            <div class="grid-box"><div>Users</div><div class="grid-val">{{ users|length }}</div></div>
            <div class="grid-box" style="border-color:var(--green);"><div>Online</div><div class="grid-val" style="color:var(--green);">{{ active_count }}</div></div>
            <div class="grid-box full"><div>Uptime</div><div class="grid-val" style="color:var(--purple);">{{ uptime }}</div></div>
            <div class="grid-box" style="border-color:#3498db;"><div>Bandwidth</div><div class="grid-val" style="color:#3498db;">{{ total_usage }}</div></div>
            <div class="grid-box" style="border-color:#e67e22;"><div>Load</div><div class="grid-val" style="color:#e67e22;">12%</div></div>
        </div>
        <div class="card">
            <form method="post" action="/add">
                <input name="user" placeholder="Name" required><input name="password" placeholder="Pass" required><input name="days" placeholder="Days" required><button class="main-btn">CREATE USER</button>
            </form>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><form method="post" action="/delete" style="display:inline;" onsubmit="return confirm('Delete?')"><input type="hidden" name="user" value="{{u.user}}"><button type="submit" class="delete-btn"><i class="fas fa-trash-alt"></i></button></form></td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
...
<script>
    ...
    function anim() { ctx.clearRect(0,0,canvas.width,canvas.height); hue += 0.5; pts.forEach((p,i)=>{ p.up(); p.dr(); for(let j=i+1;j<pts.length;j++){ let d = Math.hypot(p.x-pts[j].x, p.y-pts[j].y); if(d<125){ ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(pts[j].x,pts[j].y); ctx.strokeStyle='hsla('+(hue + d)+', 70%, 60%, '+(1-d/125)*0.8+')'; ctx.lineWidth=0.8; ctx.stroke(); } } }); requestAnimationFrame(anim); } anim();
</script></body></html>"""
