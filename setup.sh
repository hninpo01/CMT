...
def start_tg_bot():
    ...
    try:
        ...
        def bot_add(update, context):
            if str(update.effective_chat.id) != get_env("TG_CHAT_ID"): return
            try:
                ...
                update.message.reply_text(f"✅ User {u} added for {d} days!")
            except: update.message.reply_text("Usage: /adduser [name] [days]")
...
{% else %}
    ...
    <div id="renewModal" class="modal"><div class="modal-content">
        ...
        <form method="post" action="/renew"><input type="hidden" name="user" id="renewUser"><input name="days" placeholder="ရက်ပေါင်း" required><button class="main-btn">အတည်ပြုသည်</button></form>
...
echo -e "\n✅ Installation Fixed! http://$IP:8080"
