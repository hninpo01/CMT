#!/bin/bash
# CMT ZIVPN PRO - FINAL ULTIMATE FIX
set -euo pipefail
...
<head>
    ...
    <style>
        ...
        .title-container { text-align: center; padding: 20px 0; border-bottom: 2px solid var(--cyan); background: rgba(0,0,0,0.6); backdrop-filter: blur(10px); }
        .header { background: rgba(0,0,0,0.5); padding: 15px; display: flex; align-items: center; justify-content: space-between; }
        .header img { border-radius: 50%; width: 45px; height: 45px; background: #fff; }
        .clock-box { text-align: right; margin-right: 15px; }
        .clock-time { font-size: 1.1em; font-weight: bold; }
        .social-row { display: flex; gap: 10px; }
        .btn-social { width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; border-radius: 50%; color: white; text-decoration: none; font-size: 1.2em; background: #444; }
        .container { padding: 15px; }
        .grid-menu { display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-bottom: 20px; }
        .grid-box { background: var(--card); border: 2px solid var(--glow); border-radius: 12px; padding: 12px; text-align: center; }
        ...
        input { width: 100%; padding: 12px 15px; margin: 10px 0; background: linear-gradient(90deg, #ff000022, #00d4ff22); color: #fff; border: 1.5px solid var(--cyan); border-radius: 10px; outline: none; }
        .main-btn { background: linear-gradient(90deg, #ff0000, #ffaa00, #00d4ff); background-size: 300%; animation: rainbowBG 4s linear infinite; color: #fff; border: none; padding: 12px; border-radius: 10px; font-weight: bold; width: 100%; cursor: pointer; }
        .table-card { background: var(--card); border-radius: 12px; border: 1.5px solid var(--cyan); overflow-x: auto; }
        table { width: 100%; border-collapse: collapse; min-width: 500px; }
        th, td { padding: 12px; text-align: left; border-bottom: 1px solid #1e293b; }
        .delete-btn { color: #ff4444; background: none; border: none; cursor: pointer; font-size: 1.1em; }
...
{% if not session.get('auth') %}
    <div style="max-width: 320px; margin: 20vh auto; background: var(--card); padding: 35px; border-radius: 20px; text-align: center; border: 3px solid var(--glow);">
        ...
        <form method="post" action="/login_check"><input name="u" placeholder="Admin" required><input name="p" type="password" placeholder="Pass" required><button class="main-btn" style="margin-top:15px;">LOGIN</button></form>
    </div>
{% else %}
    ...
    <div class="header">
        <div style="display:flex;align-items:center;gap:12px;">
            <img src="{{ logo }}">
            <div class="clock-box">
                <div id="liveTime" class="rainbow-text clock-time"></div>
                <div id="liveDate" style="font-size:0.7em; color:#aaa;"></div>
            </div>
        </div>
    ...
    <div class="container">
        <div style="text-align:center; margin-bottom:15px; background:rgba(0,0,0,0.5); padding:8px; border-radius:10px; border:1px solid var(--cyan); font-size:0.8em;">SERVER IP: <span id="sip">{{ ip }}</span> <i class="fas fa-copy" style="cursor:pointer;" onclick="copyText('sip')"></i></div>
        <div class="grid-menu">
            <div class="grid-box"><div style="font-size:0.7em;">Users</div><div class="grid-val">{{ users|length }}</div></div>
            ...
            <div class="grid-box full"><div>Uptime: <span style="color:var(--purple);">{{ uptime }}</span></div></div>
        </div>
        <div class="card">
        ...
        <div class="table-card">
            <table>
                <thead><tr><th>USER</th><th>PASS</th><th>USAGE</th><th>EXPIRY</th><th>ACTION</th></tr></thead>
                <tbody>
                    {% for u in users %}
                    <tr>
                        ...
                        <td>{{ u.expires }}</td>
                        <td><form method="post" action="/delete" style="display:inline;"><input type="hidden" name="user" value="{{u.user}}"><button type="submit" class="delete-btn"><i class="fas fa-trash-alt"></i></button></form></td>
                    </tr>
                    {% endfor %}
                </tbody>
            </table>
    ...
    <div class="bottom-nav"><a href="/" style="color:var(--cyan);"><i class="fas fa-home"></i></a><a href="/logout" style="color:#555;"><i class="fas fa-power-off"></i></a></div>
{% endif %}
<script>
    function copyText(id) { var text = document.getElementById(id).innerText; navigator.clipboard.writeText(text); alert("Copied: " + text); }
    function startClock() { setInterval(function() { var now = new Date(); now.setMinutes(now.getMinutes() + now.getTimezoneOffset() + 390); var h = now.getHours(), m = now.getMinutes(), s = now.getSeconds(), ampm = h >= 12 ? 'PM' : 'AM'; h = h % 12; h = h ? h : 12; h = h < 10 ? '0'+h : h; m = m < 10 ? '0'+m : m; s = s < 10 ? '0'+s : s; document.getElementById('liveTime').innerHTML = h + ':' + m + ':' + s + ' ' + ampm; var ds = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'], ms = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; document.getElementById('liveDate').innerHTML = ds[now.getDay()] + ', ' + now.getDate() + ' ' + ms[now.getMonth()] + ' ' + now.getFullYear(); }, 1000); }
    const canvas = document.getElementById('bgCanvas'), ctx = canvas.getContext('2d');
    let pts = [], hue = 0;
    function init() { canvas.width = window.innerWidth; canvas.height = window.innerHeight; }
    window.onresize = init; init();
    class Pt { constructor() { this.x = Math.random()*canvas.width; this.y = Math.random()*canvas.height; this.vx = (Math.random()-0.5)*1.0; this.vy = (Math.random()-0.5)*1.0; this.radius = Math.random()*2 + 1; } up() { this.x+=this.vx; this.y+=this.vy; if(this.x<0||this.x>canvas.width)this.vx*=-1; if(this.y<0||this.y>canvas.height)this.vy*=-1; } dr() { ctx.beginPath(); ctx.arc(this.x,this.y,this.radius,0,Math.PI*2); ctx.fillStyle='rgba(255, 255, 255, 0.15)'; ctx.fill(); } }
    for(let i=0;i<60;i++) pts.push(new Pt());
    function anim() { ctx.clearRect(0,0,canvas.width,canvas.height); hue += 0.5; pts.forEach((p,i)=>{ p.up(); p.dr(); for(let j=i+1;j<pts.length;j++){ let d = Math.hypot(p.x-pts[j].x, p.y-pts[j].y); if(d<110){ ctx.beginPath(); ctx.moveTo(p.x,p.y); ctx.lineTo(pts[j].x,pts[j].y); ctx.strokeStyle='hsla('+(hue + d)+', 70%, 60%, '+(1-d/110)*0.7+')'; ctx.lineWidth=0.7; ctx.stroke(); } } }); requestAnimationFrame(anim); } anim();
</script></body></html>"""
...
echo -e "Web Interface: http://$IP:8080"
