#!/bin/bash
# CMT ZIVPN PRO - ULTIMATE LOGIC COMBINED (STABLE)
set -euo pipefail
...
# ✅ Admin Login (ဒီမှာ စကားဝှက် ပြောင်းထားနိုင်ပါတယ်)
...
<head>
    ...
    <style>
        ...
        .btn-tg { background: #0088cc; padding: 6px 12px; border-radius: 8px; color: white; text-decoration: none; font-size: 0.8em; }
        ...
        .usage { color: var(--yellow); font-weight: bold; }
        .status-pill { padding: 4px 8px; border-radius: 12px; font-size: 0.8em; font-weight: bold; }
        .expired-row { background: rgba(155, 89, 182, 0.15) !important; color: #ddd; }
        .bottom-nav { position: fixed; bottom: 0; left: 0; width: 100%; background: #0a0e1a; display: flex; justify-content: space-around; padding: 12px 0; border-top: 2px solid #4e73df; }
...
{% else %}
    <div class="header">
        ...
        <div style="text-align:right; display:flex; flex-direction:column; gap:5px;">
            <a href="https://t.me/Zero_Free_Vpn" target="_blank" class="btn-tg"><i class="fab fa-telegram-plane"></i> Contact</a>
            <small>IP: <span id="srvIp">{{ ip }}</span> <i class="fas fa-copy" style="cursor:pointer; color:var(--glow);" onclick="copyText('srvIp')"></i></small>
        </div>
    </div>
    <div class="container">
        <div class="grid-menu">
            ...
            <div class="grid-box" style="border-color:#3498db;"><div class="grid-label">Bandwidth</div><div class="grid-val" style="color:#3498db; text-shadow: 0 0 10px #3498db;">{{ total_usage }}</div></div>
        ...
        <div class="card">
            ...
            <form method="post" action="/add"><input name="user" placeholder="နာမည်" required><input name="password" placeholder="စကားဝှက်" required><input name="days" placeholder="ရက်ပေါင်း (ဥပမာ 30)" required><button class="btn">CREATE & SYNC</button></form>
        </div>
        <div class="table-card">
            <table>
                ...
                <tbody>
                    {% for u in users %}
                    <tr class="{{ 'expired-row' if u.is_expired }}">
                        <td style="color:var(--cyan); font-weight:bold;">{{ u.user }}</td>
                        <td><span id="pw_{{u.user}}">{{ u.password }}</span> <i class="fas fa-copy" style="cursor:pointer; font-size:0.8em; color:var(--cyan);" onclick="copyText('pw_{{u.user}}')"></i></td>
                        <td class="usage">{{ u.usage }}</td>
                        <td style="color:#ff69b4;">{{ u.expires }}</td>
                        <td>
                            {% if u.is_expired %}<span class="status-pill" style="background:var(--purple);">EXPIRED</span>
                            {% elif u.online %}<span class="status-pill" style="background:var(--green);">ONLINE</span>
                            {% else %}<span class="status-pill" style="background:var(--glow);">OFFLINE</span>{% endif %}
                        </td>
...
<script>
    function copyText(id) {
        ...
        document.body.removeChild(elem); alert("Copied: " + text);
    }
    ...
    for u in users:
        ...
        if u.get("expires"):
            try:
                ...
                if exp_date < today: u["is_expired"] = True
            except: pass
...
echo -e "\n✅ Ultimate Logic Combined! http://$(hostname -I | awk '{print $1}'):8080"
